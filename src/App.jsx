  ...
  return (
    <>
      ...
      <div className="flex justify-center mt-10   p-5">
        <form className=" w-1/3 p-4 flex flex-col gap-5 " onSubmit={handleSubmit}> 
          <input
            id="title"
            type="text"
            value={title}
          ...
          />
          <button className="bg-green-500 p-2 rounded-lg font-semibold hover:bg-green-400"  >
            Add
          </button>
        </form>
      </div>
      ...
      <div className=" m-3  p-5">
        <h1 className="text-white text-center text-3xl font-bold pb-4 ">Here is your task list</h1>
        {/* display of task starts from here */}
        {tasks.length == 0 && <p className="text-center mt-3 text-white font-2xl"> No pending task so far </p>}
        {tasks.length > 0 && (
          <div className="grid grid-cols-5 gap-5">
            {tasks.map((task)=>(
              <div key={task.id} className={`flex flex-col border p-3 rounded-lg relative ${task.completed ? "border-green-500 bg-green-500/80" : "border-red-500"}`}>
                <p className="font-bold text-white text-lg">{task.title}</p>
                <p className="font-gray text-white text-sm">{task.description}</p>
                <button onClick={() => toggleCompleted(task.id)} className="w-fit px-2 py-1 mt-2 border border-gray-500 bg-white/90 text-black text-xs rounded-lg hover:bg-white/70">{task.completed ? 'Undone' : 'Done'}</button>
                <p className="absolute top-3 right-4 cursor-pointer text-white" onClick={() => deleteTask(task.id)}>⛌</p>
              </div>
            ))}
